...
class ChatBot(models.AbstractModel):
    ...
    def reply_from_bot(self, record, values, command=None):
        bot_id = self.env['ir.model.data']._xmlid_to_res_id("bayvista_realestate.bot_partner")
    ...
    def _get_answer(self, record, body, values, command):
        try:
            ...
            response = openai.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[{"role": "user", "content": body + " Please give the short answer. i don't need any description. And make the answer as short as possible."}],
                max_tokens=500
            )
    ...
    def search_odoobot_channel(self):
        odoobot_partner_id = self.env['ir.model.data']._xmlid_to_res_id("bayvista_realestate.bot_partner")
